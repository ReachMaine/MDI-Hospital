"use strict";
	// Aug2021 zig change .live to .on
jQuery(document).ready(function() {
//	simple_select_2_init();
	invokeSelect2();
	pinterest_layout_select();
	jQuery(".redux-image-select#cws-mb-sb_layout .redux-image-select").on('click',pinterest_layout_select);
});
...
function invokeSelect2() {
	...
	jQuery('.show_icon_options').on('change', function (){
	...
	jQuery('a[id^="media"]').on("click", function() {
	...
	jQuery('a[id^="remov"]').on("click",function()
	{
		var _this = jQuery(this).attr('id').substring(6);
		jQuery("#"+_this).attr("value", '');
		jQuery('img#img-'+_this).hide();
