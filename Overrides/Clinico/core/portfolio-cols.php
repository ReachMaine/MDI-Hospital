<?php
/*
	8Jun15 remove hover effect on staff picts & make pics clickable
	7July15 zig:  order the carosel by title
	17jul15 zig :  order render_portfolio by title asc - removed 21Aug15, using post order plugin
*/
	...
	while ($r->have_posts()):
		...
		if ($is_dir) {
			...
			if ( has_post_thumbnail(get_the_ID()) ) {
				$out .= "<!-- zig:  photo ".get_the_ID()."-->";
				$out_hold .= build_portfolio_item( get_the_ID(), 4, true, $thumbnail_dims, $chars_count, $cp_type );
			} else {
				$out .= "<!-- zig: no photo ".get_the_ID()."-->";
				$out_hold_last .= build_portfolio_item( get_the_ID(), 4, true, $thumbnail_dims, $chars_count, $cp_type );
			}
		}
			
	endwhile;
	$out .= $out_hold.$out_hold_last."</div></div>";
	wp_reset_postdata();
	return $out;
}
...
function build_portfolio_item ($pageid, $cols, $is_carousel = false, $thumbnail_dims, $chars_count, $cp_type ) {
	...
	switch ($cp_type) {
		...
		case 'staff':
			...
			$out .= '<a class="zhover-effect" href="'.$link.'">'; // zig
			$out .= '<img src="' . bfi_thumb($img[0], array('width' => 270, 'height' => 270) ) . '" alt="">';
			$out .= '</a>';
			//$out .= '<div class="hover-effect"></div>'; // zig
			$out .= '<div class="links">';
