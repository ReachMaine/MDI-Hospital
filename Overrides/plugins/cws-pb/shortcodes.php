...
if ( !shortcode_exists( 'cws-widget' ) ) {
	function shortcode_cws_widget ( $atts, $content ){
		...
		$args = array( 'atts' => $original_att, 'title'=> (isset($original_att['title']) ? $original_att['title'] : ''), 'content' => $content );
...
if (!$pbf_redeclares || !in_array('cws_accordion_item_renderer', $pbf_redeclares) ) {
	function cws_accordion_item_renderer ( $title, $content, $open, $iconfa, $iconimg ){
		...
		$iconfa5 = substr($iconfa, 0, 5);
		if (!empty( $iconfa )) {
			if ('fa fa' !== $iconfa5 && 'flati' !== $iconfa5) {
				$iconfa = 'fa fa-' . $iconfa;
			} else {
				$iconfa = 'fa ' . $iconfa;
			}
		...
		$out .= "<div class='accordion_title'>" . ( $featured ? ( !empty( $iconfa ) ? "<i class='acc_featured_icon $iconfa'></i>" : ( !empty( $iconimg ) ? "<span class='acc_featured_img'><img width='$iconimg_size' height='$iconimg_size' src='" . ( bfi_thumb( $iconimg, array( 'width' => $iconimg_size . 'px', 'height' => $iconimg_size . 'px' ) ) ) . "' /></span>" : "" ) ) : "" ) . "$title<button class='accordion_icon'></i></div>"; // TITLE
