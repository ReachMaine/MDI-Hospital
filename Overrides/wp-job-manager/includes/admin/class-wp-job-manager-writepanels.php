<?php
/* 10Sept15 zig add defaults for some values for MDIH */
...
class WP_Job_Manager_Writepanels {
	...
	public function job_listing_fields() {
		...
		// here are the defaults - zig
		$zjd_default_location = "10 Wayman Lane, Bar Harbor, ME";
		$zjd_default_URL = site_url()."/jobs/apply-now/";
		$zjd_default_logo = site_url()."/wp-content/uploads/2015/09/jobs-logo.jpg"; //get_stylesheet_directory_uri();
		$zjd_default_company_name = "MDI Hospital";
		...
		$fields = array(
			'_job_location' => array(
				...
				'value'       => metadata_exists( 'post', $post->ID, '_job_location' ) ? get_post_meta( $post->ID, '_job_location', true ) : $zjd_default_location,
				'priority'    => 1
			),
			'_application' => array(
				...
				'value'       => metadata_exists( 'post', $post->ID, '_application' ) ? get_post_meta( $post->ID, '_application', true ) : $zjd_default_URL ,
				'priority'    => 2
			),
			'_company_name' => array(
				'label'       => __( 'Company Name', 'wp-job-manager' ),
				'value'       => metadata_exists( 'post', $post->ID, '_company_name' ) ? get_post_meta( $post->ID, '_company_name', true ) : $zjd_default_company_name ,
				'placeholder' => '',
				'priority'    => 3
			),
			...
			'_company_tagline' => array(
				...
				'placeholder' => __( 'Department', 'wp-job-manager' ),
				'priority'    => 5
			),
			...
			'_company_logo' => array(
				...
				'type'        => 'file',
				'value'       => metadata_exists( 'post', $post->ID, '_company_logo' ) ? get_post_meta( $post->ID, '_company_logo', true ) : $zjd_default_logo ,
				'priority'    => 7
			),
		...
		if ( $current_user->has_cap( 'manage_job_listings' ) ) {
			...
			$fields['_job_expires'] = array(
				...
				'priority'    => 11,
				'value'       => metadata_exists( 'post', $post->ID, '_job_expires' ) ? get_post_meta( $post->ID, '_job_expires', true ) : "",
			);
		}
