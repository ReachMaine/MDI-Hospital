...
function reach_filter_content($in_content, $post) {
  ...
  if ($in_content) {
    if ($in_content) {
    $target = get_the_permalink($post);
    $out_content .= '<a class="cws_button small" href="'.$target.'">See More </a>';
        /* $out_content .= '[cws-widget type=accs title="" toggle=1  items=1][item type=accs title="See More"]';
