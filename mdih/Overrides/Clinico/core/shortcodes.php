<?php
/*  Mods
	zig 16Jan19 use button for accordion open/close for WAC
*/
...
function shortcode_services($attr) {
	...
	if (0 !== count($depts) ) {
		...
		foreach ($depts as $dept=>$v) {
			...
			$out .= sprintf('<div id="cws-service-%s" class="accordion_title">%s%s<button class="accordion_icon"></button></div>', $v->term_id, $fa_check, $v->name);
