...
function dept_metabox_add($tag) {
	...
	<?php /* zig add more link & map field */ ?>
	<div class="form-field">
	...
	<div class="form-field">
	<label for="cws-clinico-dept-map"><?php _e('Map', THEME_SLUG)?></label>
	<textarea name="cws-clinico-dept-map" id="cws-clinico-dept-map" class="postform" rows="1" cols="40"></textarea>
	</div>
	<?php /* zig end */ ?>
	<div class="form-field">
...
function dept_metabox_edit($tag) {
	...
	$morelink = get_option_value('cws-clinico-dept-more',  $tag->term_id);
	$map = get_option_value('cws-clinico-dept-map',  $tag->term_id);
	...
	<tr class="form-field">
		<th scope="row" valign="top">
			...
			<textarea name="cws-clinico-dept-map" id="cws-clinico-dept-map" class="postform" rows="1" cols="40"><?php echo $map; ?></textarea>
		</td>
	</tr>
	<?php /* zig end */ ?>
	<?php
...
function staff_metabox_save( $term_id, $tt_id ) {
	...
	if ( isset( $_POST['cws-clinico-dept-more'] ) ) { // zig
		$option = get_option('cws-clinico-dept-more');
		$option[$term_id] = $_POST['cws-clinico-dept-more'];
	} else {
		$option[$term_id] = '';
	}
	update_option( 'cws-clinico-dept-more', $option);

	if ( isset( $_POST['cws-clinico-dept-map'] ) ) { // zig
		$option = get_option('cws-clinico-dept-map');
		$option[$term_id] = $_POST['cws-clinico-dept-map'];
	} else {
		$option[$term_id] = '';
	}
	update_option( 'cws-clinico-dept-map', $option);
...
function cws_mb_staff_callback ( $post ) {
	...
	$ed_residency = isset( $cws_stored_meta['cws-staff-residency'] ) ? esc_attr( $cws_stored_meta['cws-staff-residency'] ) : '';//zig
	$ed_fellowship = isset( $cws_stored_meta['cws-staff-fellowship'] ) ? esc_attr( $cws_stored_meta['cws-staff-fellowship'] ) : '';//zig
	...
	$boardcert = isset( $cws_stored_meta['cws-staff-boardcert'] ) ? esc_attr( $cws_stored_meta['cws-staff-boardcert'] ) : ''; //zig
	$post_mb_staff_attr = array(
		'staff'=>array(
			'options'=>array(
				array(
					'id' => 'degree',
					'title' => __('Education / Med School',THEME_SLUG), // zig changed title
					'type' => 'text',
					'default' => $degree,
					'w' => '100%'
				),
				array(
					'id' => 'residency',
					'title' => __('Residency  (MDs only)',THEME_SLUG),// zig changed title
					'type' => 'text',
					'default' => $ed_residency,
					'w' => '100%'
				),
				array(
					'id' => 'fellowship',
					'title' => __('Fellowship  (MDs only)',THEME_SLUG),// zig changed title
					'type' => 'text',
					'default' => $ed_fellowship,
					'w' => '100%'
				),
				array(
					'id' => 'office',
					'title' => __('Contact',THEME_SLUG),
					'type' => 'text',
					'default' => $office,
					'w' => '100%'
				),
				array(
					'id' => 'boardcert',
					'title' => __('Board Certification',THEME_SLUG),
					'type' => 'text',
					'default' => $boardcert,
					'w' => '100%'
				),
